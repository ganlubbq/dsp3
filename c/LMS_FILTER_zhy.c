...
double updatecoeff(double *hr,double *hi, int Ntap,
               ...
               bool dontskip, int N)
{
    ...
    double sum, real, imag, errx, erry;
    for(i=0;i<Ydim;i++)
    {
        ...
        *(yi+i+Ydim) = vmac(xi+i,h2r,Ntap) + vmac(xr+i,h2i,Ntap) 
        ...
        {
            errx=updatecoeff(h1r,h1i,Ntap,xr+i,xi+i,Ndim,*(yr+i),     *(yi+i),     mu,Mr,Mi,N);
            erry=updatecoeff(h2r,h2i,Ntap,xr+i,xi+i,Ndim,*(yr+i+Ydim),*(yi+i+Ydim),mu,Mr,Mi,N);
        }
