...
for ii = 0:length(betap)-1;
    halfstep = halfstep - 1j*betap(ii+1)*(w).^ii/factorial(ii);
end
...
for iz = 1:nz,
    ...
    for ii = 1:maxiter,
        uv = uhalf .* exp(-1j*gamma*(abs(u1).^2 + abs(u0).^2)*dz/2);
        uv = fft(uv);
        ufft = halfstep.*uv;
        uv = ifft(ufft);
    ...
    if (ii == maxiter)
        strErr = sprintf('Failed to converge to %f in %d iterations',...
            tol,maxiter);
        warning('ssprop:ssf:converge',strErr)
    end
    u0 = u1;
end
