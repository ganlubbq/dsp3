function [x_out,mse,deth,h1,h2] = mimoEqualizer( x_in,mn,sps,polm,mu, ntaps, errid, iter, appLMS)
% Time domain MIMO FIR filter for polarization demultiplexing and channel
% equalization, including constant modulus algorithm (CMA) and least mean
...
if size(x_in,2)~=2
    error('MIMO input should have two columns');
end

x = normalize_mqam(x_in, mn);
...
if appLMS
    for ii = 1:iter(1)
        [xx mse deth] = LMS_FILTER(extendx,h1,h2,nt,mu(1),cstl,sps);
    end
    x_out = xx(  1:sps:end,:);
    mse  = mse( 1:sps:end  );
    deth = deth(1:sps:end  );
...
for stage = 1:3
    ...
    for ii = 1:iter(stage)
        [xx mse deth] = CMA_FILTER(extendx,h1,h2,nt,mu(stage),cm,sps,errid(stage),stage,method);
    end
end
